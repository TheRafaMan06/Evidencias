...
def RankingClientes():
    try:
        ...
        # Realizar el merge con los datos de los clientes
        ...
        df_ranking['Nombre Completo'] = df_ranking['nombres'] + ' ' + df_ranking['apellidos']
        df_ranking = df_ranking[['Cantidad de Prestamos', 'clave_cliente', 'Nombre Completo', 'telefono']]
        ...
        if df_ranking.empty:
            ...
            print("Ranking de Clientes (ordenado por cantidad de rentas acumuladas de forma descendente):")
            for index, row in df_ranking.iterrows():
                print(f"{index + 1}. Rentas: {row['Cantidad de Prestamos']}, "
                    f"Clave: {row['clave_cliente']}, "
                    f"Nombre: {row['Nombre Completo']}, "
                    f"Teléfono: {row['telefono']}")

            # Exportar el reporte
            ExportarReporte(df_ranking.values, 
                            ['Rentas Acumuladas', 'Clave Cliente', 'Nombre Completo', 'Teléfono'], 
                            "ranking_clientes")
